...
def build_html(articles):
    ...
    for i, art in enumerate(articles):
        ...
        <div style="background:#fff;border:0.5px solid #E5E5E0;border-radius:12px;
          ...
          <div style="padding:16px 18px;">
            <div style="font-size:11px;color:#EA6A1F;font-weight:500;margin-bottom:6px;">
              {art.get('source','엔지니어링데일리')}
            </div>
            <div style="font-size:16px;font-weight:500;color:#2C2C2A;
            ...
            <a href="{art['url']}" style="display:inline-block;background:#FAEEDA;
               color:#854F0B;padding:6px 14px;border-radius:6px;font-size:12px;
    ...
    return f"""<!DOCTYPE html><html lang="ko"><body
      ...
      <div style="max-width:600px;margin:0 auto;background:#fff;
                  border-radius:16px;overflow:hidden;border:0.5px solid #E5E5E0;">

        <div style="background:#EA6A1F;padding:32px 28px;text-align:center;color:#fff;">
          <div style="font-size:11px;letter-spacing:4px;opacity:0.85;margin-bottom:8px;">
            DAILY NEWSLETTER
          </div>
          <div style="font-size:32px;font-weight:500;letter-spacing:-0.5px;margin-bottom:6px;">
            선진레터
          </div>
          <div style="font-size:13px;opacity:0.9;">
            {today} · {len(articles)}건
          </div>
        </div>
...
def send_email(articles):
    ...
    msg["Subject"] = f"[선진레터] {today} — {len(articles)}건"
    msg["From"] = f"선진레터 <{EMAIL_ADDRESS}>"
